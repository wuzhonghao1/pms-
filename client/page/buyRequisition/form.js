    ...
    approve = async (type)=>{
        ...
        if(type === "agree"){
            ...
            if(opinion){
                let response = await window.$ajax.post(URL.updatePerform,{
                    serialNumber:this.state.serialNumber,//url中获取
                    processType: this.state.data ? this.state.data.PROCESS_TYPE:"",//流程类型 detail
                    processId:this.state.processId,//流程id detail
                    approvalStatus: '1',//定死
                    opinion: opinion?opinion:"", //原因
                    personId: this.state.personId ? this.state.personId:"",//approver --审核人的personId
    ...
    render() {
        ...
        return (
            <div>
                ...
                />
                <List>
                    <Item wrap={true} extra={this.state.data ? this.state.data.APPLICANT : null}>申请人</Item>
                    <Item wrap={true} extra={this.state.data ? this.state.data.REQUISITIONS_NUMBER : null}>编号</Item>
                    ...
                    <Item wrap={true} align={"top"} extra={this.state.data ? this.state.data.REQUISITION_REASONS : null}>申请原因</Item>
                </List>
